/*
    ...
    with a cloud update service to check for, download, verify, and apply device firmware updates.
    Compatible with the EmbedThis Builder service by default, or any custom backend implementing the
    update protocol.
...
/**
    ...
    @description This function implements a complete OTA update workflow:
        1. Queries the update service to check for available updates
    ...
    All parameters must be non-NULL except for 'properties', 'script', 'checkPath', and 'reportPath'.

    @param host Update service endpoint URL. Must not be NULL.
    @param product Product identifier. Must not be NULL.
    @param token API access token for authentication. Must not be NULL.
    ...
    @param checkPath API path for update check requests. If NULL, defaults to "/tok/provision/update" for
        EmbedThis Builder compatibility. May be NULL.
    @param reportPath API path for update status report requests. If NULL, defaults to "/tok/provision/updateReport"
        for EmbedThis Builder compatibility. May be NULL.
...
PUBLIC int update(cchar *host, cchar *product, cchar *token, cchar *device, cchar *version, cchar *properties,
                  cchar *path, cchar *script, int verbose, int quiet, cchar *checkPath, cchar *reportPath);
